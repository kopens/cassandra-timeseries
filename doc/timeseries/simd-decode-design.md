...
| 벤치마크 | 결과 |
| --- | --- |
| `ChunkReadBench.fullScan` (이름 기반, 종전) | 812.1 ± 20.6 µs/op |
| `ChunkReadBench.fullScanBySlot` (슬롯 기반) | **432.9 ± 19.7 µs/op** |

**1.88×, 청크 전체 스캔의 47%.** 신뢰구간이 겹치지 않는다.

따라서 부수 발견 1의 지분 계산은 갱신되어야 한다: 블록 수학 ~60µs는 이제 812가 아니라 **433µs
기준으로 ~14%**다. 벡터 커널 보류 판정 자체는 바뀌지 않는다(여전히 15% 임계값 부근이고, 지배
