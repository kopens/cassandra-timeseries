...
**현황 (2026-08-28 실측):** 운영 노드(41)는 **v6.0.0-r1**로 돌고 있고(2026-08-28 배포, 첫 CI
게이트 통과 릴리스), 계층화는 **운영 테이블에 이미 켜져 있습니다** — `pp.tm_tag_point`(윈도우
...
테이블에 켜기 전 점검이자 운영 중 참조**로 읽어야 합니다. v6.0.0-r1 배포로 8/7 빌드에 없던
...
- **HA 워치독의 자동 재기동을 전제하지 마십시오.** 이 문서는 원래 `plantpulse-ha`가 정지된
  카산드라를 ~30초 만에 자동 재기동한다고 적었지만, **2026-08-28 배포에서는 재기동하지 않았습니다**
  — drain+stop 후 16분간 내려가 있다가 수동 `start.sh`로 올렸습니다(HA 프로세스는 떠 있었음).
  따라서 절차는 그대로 **새 jar을 `lib/`에 먼저 넣고 나서** 내리되(HA가 올릴 경우를 대비한 순서는
  유지), **정지 후에는 자동 재기동을 기다리지 말고 직접 `start.sh`로 올린 뒤 확인**하십시오.
  HA가 왜 재기동하지 않았는지는 미해결 — 다음 유지보수 때 HA의 감시 대상을 확인할 것.
...
- **운영 백업이 `<테이블>__chunks`를 포함하는지 확인된 적이 없습니다.** 계층화가 이미 운영
  테이블에 켜져 있으므로(문서 첫머리 현황), 인코딩된 구간의 유일한 사본은 청크 테이블입니다.
  백업 정책 점검이 §0.2의 요구 중 유일하게 남은 미확인 항목입니다.
